    ...
    Private Sub RequestsGrid_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles RequestsGrid.CellContentClick
        ...
        Try
            ...
            If action = "Approve" Then
                ...
                Using updateAssignment As New OleDbCommand("UPDATE Assignments SET Status = 'Approved' WHERE ID = ?", conn)
                ...
                Using getAssetIDCmd As New OleDbCommand("SELECT AssetID FROM Assignments WHERE ID = ?", conn)
                    getAssetIDCmd.Parameters.AddWithValue("?", requestID)
                    assetID = CInt(getAssetIDCmd.ExecuteScalar())
                End Using

                ' Mark asset as assigned
                Using updateAsset As New OleDbCommand("UPDATE Assets SET Assigned = True WHERE AssetID = ?", conn)
            ...
            ElseIf action = "Reject" Then
                Using rejectCmd As New OleDbCommand("UPDATE Assignments SET Status = 'Rejected' WHERE ID = ?", conn)
        ...
        ' ✅ Reload after closing connection
        LoadRequests()
    End Sub
